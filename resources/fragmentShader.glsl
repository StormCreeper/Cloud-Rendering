...
Voxel sampleVoxelGrid(VoxelMap map, int x, int y, int z) {
	...
	Voxel vox;
	vox.color = texture(map.colorTex, vec3(x + .5, y + .5, z + .5) / vec3(map.size)).rgb;
	vox.normal = normalize(texture(map.normalTex, vec3(x + .5, y + .5, z + .5) / vec3(map.size)).rgb * 2.0f - 1.0f);
	return vox;
}
